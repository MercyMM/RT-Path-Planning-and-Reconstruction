...
int attitudeControl(CoreAPI* api, Flight* flight, float32_t rollDesired, float32_t pitchDesired, float32_t yawDesired, int timeoutInMs, float thresholdInDeg) //Holds current height
{
  ...
  while(std::abs(curEuler.roll - rollDesiredRad) > thresholdInRad || \
        std::abs(curEuler.pitch - pitchDesiredRad) > thresholdInRad || \
        std::abs(curEuler.yaw - yawDesiredRad) > thresholdInRad)
  {
...
//    cout<<"x:" << xCmd <<" y:" << yCmd<< " z:"<< zCmd << endl;
    ...
    localOffsetFromGpsOffset(curLocalOffset, &curPosition, &originPosition);
//    printf("long: %lf, lat: %lf, curx: %lf, cury: %lf\n",\
//           curPosition.longitude, curPosition.latitude,curLocalOffset.x, curLocalOffset.y );
cout<<curPosition.longitude << "  ";
cout<<curPosition.latitude << "  ";
cout<<curLocalOffset.x << "  ";
cout<<curLocalOffset.y << "  ";
cout<<endl;
...
!*/
int moveWithVelocity(CoreAPI* api, Flight* flight, float32_t xVelocityDesired, float32_t yVelocityDesired, \
                     float32_t zVelocityDesired, float32_t yawRateDesired ,  int timeoutInMs, \
                     float yawRateThresholdInDegS, float velThresholdInMs)
{
  ...
  int elapsedTime = 0;
    velThresholdInMs = 0.1;
//  printf(" curVelocity.x = %f, velThresholdInMs = %f\n", curVelocity.x ,velThresholdInMs);
  int count = 0;
  ...
  {
//  printf(" curVelocity.x = %f, curVelocity.y = %f, curW.z = %f\n", curVelocity.x ,curVelocity.y, curW.z);
    if (elapsedTime >= timeoutInMs)
      break;
    count ++;
//    cout<<"move With V"<<endl;
    flight->setMovementControl(flag, xVelocityDesired, yVelocityDesired, zVelocityDesired, yawRateDesired);
    usleep(20000); // 10000 ns;????
    elapsedTime += 20;
//    yawRateDesired = 0;
    ...
    PositionData* origin)
{
  ...
  deltaNed.z = target->height - origin->height;
    printf("deltaLat: %lf, x: %lf  ", deltaLat, deltaNed.x);
}
